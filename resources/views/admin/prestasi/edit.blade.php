        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    <h1 class="mt-4">Edit Prestasi Siswa</h1>
                            ...
                            <form action="{{ route('admin.prestasi.update', $prestasi->prestasi_id) }}" method="POST">
                                    ...
                                    <select class="form-select @error('siswa_id') is-invalid @enderror" 
                                        ...
                                        @foreach($siswa as $s)
                                            <option value="{{ $s->siswa_id }}" 
                                                {{ (old('siswa_id', $prestasi->siswa_id) == $s->siswa_id) ? 'selected' : '' }}>
                                                {{ $s->nis }} - {{ $s->nama_siswa }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select class="form-select @error('jenis_prestasi_id') is-invalid @enderror" 
                                        ...
                                        @foreach($jenisPrestasi as $jp)
                                            <option value="{{ $jp->jenis_prestasi_id }}" 
                                                    data-poin="{{ $jp->poin }}"
                                                    {{ (old('jenis_prestasi_id', $prestasi->jenis_prestasi_id) == $jp->jenis_prestasi_id) ? 'selected' : '' }}>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="date" class="form-control @error('tanggal_prestasi') is-invalid @enderror" 
                                           ...
                                           value="{{ old('tanggal_prestasi', $prestasi->tanggal_prestasi ? \Carbon\Carbon::parse($prestasi->tanggal_prestasi)->format('Y-m-d') : $prestasi->created_at->format('Y-m-d')) }}" 
                                           required>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="poin_display" readonly 
                                           value="+{{ $prestasi->poin }} poin">
                                </div>

                                <div class="mb-3">
                                    <label for="keterangan" class="form-label">Keterangan <span class="text-danger">*</span></label>
                                    <textarea class="form-control @error('keterangan') is-invalid @enderror" 
                                              id="keterangan" name="keterangan" rows="3" required>{{ old('keterangan', $prestasi->keterangan) }}</textarea>
                                    @error('keterangan')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
